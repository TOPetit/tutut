var current_thread = "4567795139935838";
var nb_log_glob = 20;
var nb_log;
var page = 1;
...
window.onload = function () {
    ...
    document.getElementById("thread").innerHTML = threads[current_thread];
    ...
    // document.getElementById("btn4").innerHTML = threads[100040105721223];
    ...
    // document.getElementById("btn4").onclick = function () { updatePage("100040105721223") };
...
function pageChange(n_page) {
    ...
    data.then(function (data) {
        page = Math.min(Math.ceil((data.split('\n').length - 1) / nb_log_glob), Math.max(1, n_page));
        document.getElementById("page_number").innerHTML = "Page " + String(page);
...
async function fetchText() {
    let response = await fetch(window.location.href + current_thread + '.csv');
...
function loading() {
    let data = fetchText();
    data.then(function (data) {
        nb_log = Math.min(data.split('\n').length - 1, nb_log_glob); // Can't show more log than what we have
        fastest(data);
        latest(data);
        error_tile(data);
        let res = serie(data, nb_log); // Needs to be behind new_log
...
function error_tile(data) {
    ...
    let nb_people = in_thread[current_thread].length;
    var values = [];
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        if (!isCorrect(line)) {
            ...
            let id = in_thread[current_thread].indexOf(parseInt(current_name))
            values[id] += 1;
...
function new_log(data, series) {
    ...
    // Creating the table
    for (let index = 0; index < Math.min(lines.length, nb_log_glob); index++) {
...
function createLogRow(index, labels, line, nb_zeros, series) {
    ...
    let data = line.split(';');
    strs[0] = String(index + nb_log_glob * (page - 1)).padStart(nb_zeros, '0');
    strs[1] = names[data[0]];
...
function tututPie(d) {
    ...
    let values = [];
    for (let index = 0; index < in_thread[current_thread].length; index++) {
        values[index] = 0;
    }
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        if (isCorrect(element)) {
            values[in_thread[current_thread].indexOf(parseInt(element.split(';')[0]))] += 1;
        }
    }

    let data = values;
    let labels = in_thread[current_thread].map(x => names[x]);
    ...
    pieChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: data,
                backgroundColor: in_thread[current_thread].map(x => colors[x]),
                hoverBackgroundColor: in_thread[current_thread].map(x => hoverColors[x]),
