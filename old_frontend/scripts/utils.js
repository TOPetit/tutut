...
var in_thread = {
    ...
    12: [100040105721223, 100014962631116]
}

var colors = {
    100032236723941: '#1e838f',
...
function medal_tile(data) {
    ...
    for (let index = 0; index < nb_people; index++) {
        ...
        data_value.innerHTML = printArray(tmp_medal[index]);
...
function sec_tile(data) {
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        if (isCorrect(line)) {
            ...
            value[name_index] += 1;
            // Add sec to total
            sec[name_index] += parsed['value'];
        }
    }

    for (let index = 0; index < nb_people; index++) {
        value[index] = (sec[index] / value[index]).toFixed(3);
    }
    ...
    let css = '#tile_sec:hover {box-shadow: 0 0 15px ' + colors[in_thread[glob_current_thread_id][indexes[0]]] + '}';
    let style = document.createElement('style');
    ...
    for (let index = 0; index < nb_people; index++) {
        ...
        let data_value = document.createElement("div");
        data_value.setAttribute("class", "value");
        data_value.innerHTML = tmp_value[index] + "s";
...
function printArray(values) {
    res = String(values[0]).padStart(3, '0');
    for (let index = 1; index < values.length; index++) {
        res += "&nbsp;&nbsp;&nbsp;" + String(values[index]).padStart(3, '0');
    }
    return res;
}